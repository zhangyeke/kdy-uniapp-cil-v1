...
// 打开授权
export function openAuthorize(scopeKey, successCall) {

	uni.getSetting({
		success: auth => {
			let flag = Object.keys(auth.authSetting).some(key => auth.authSetting[`scope.${scopeKey}`] ==
				false || auth.authSetting[`scope.${scopeKey}`] == undefined)
...
// 打开第三方APP
function openOtherAPP(url, openUrl, identity) {
...
// 打开天猫APP
export function openTM(url) {
	let openUrl = `tmall://tmallclient/?{"action":"item:id=636450346847"}`;
	// if (url.substr(0, 5) === 'https') {
	// 	openUrl = `tmall://${url.substr(8)}`;
	// } else if (url.substr(0, 4) === 'http') {
	// 	openUrl = `tmall://${url.substr(7)}`;
	// }
	openOtherAPP(url, openUrl)
}

// 打开淘宝商城APP
export function openTB(url) {
	let openUrl = url;
...
export function openPDD(url) {
	...
	if (plus.os.name == 'Android') {
		...
		openOtherAPP(url, 'pinduoduo://' + openUrl)
	}
}
